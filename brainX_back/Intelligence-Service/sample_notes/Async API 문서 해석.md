...
# 읽는 순서
1. `channels`에서 이벤트 topic 주소를 본다.
2. `x-producer-service`로 누가 발행하는지 본다.
3. `x-consumer-services`로 누가 구독하는지 본다.
4. `messages`의 $ref를 따라가서 메시지 구조를 본다.
5. `payload`의 $ref를 따라가서 실제 비즈니스 필드를 본다.
6. `x-delivery`로 중복, 순서, DLQ 정책을 확인한다.

# BrainX AsyncAPI 상세 설명
...
2. `messages`의 $ref를 따라간다.
3. `components.messages`에서 메시지 envelope를 본다.
4. `payload`의 $ref를 따라간다.
