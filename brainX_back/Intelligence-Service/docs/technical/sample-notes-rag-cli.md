...
5. RAG context에는 기본적으로 `score >= 0.35`인 chunk만 넣고, 같은 `noteId`는 최대 2개 chunk까지만 넣는다. 최종 context가 비면 LLM을 호출하지 않고 관련 chunk 없음으로 응답한다.
6. `ChatClient`가 있으면 `AiChatPort.generate(...)`로 `gpt-5.4-mini` 답변을 생성하고, 없으면 retrieval-only JSON을 반환한다.
7. `answerMode=llm`이면 chat response의 token usage를 `TokenUsagePort`로 기록하고, model catalog 단가가 있으면 estimated cost를 채운다. `answerMode=retrieval`은 provider chat 호출이 없으므로 usage record를 만들지 않는다.

`local` profile은 Voyage embedding model 단가 seed를 `ai_models` catalog에 적재한다. 단, CLI의 실제 chat/embedding model 선택은 DB catalog가 아니라 runtime 설정이 결정한다. chat usage cost는 `brainx.dev.sample-rag.chat-model`에 해당하는 catalog row가 있을 때 계산되고, embedding usage cost는 `VOYAGE_EMBEDDING_MODEL`에 해당하는 catalog row가 있을 때 계산된다.
...
`chunks.md`는 `responses.jsonl`을 사람이 읽기 쉽게 펼친 report다. 각 질의마다 반환된 모든 context chunk를 rank 순서로 표시하고, 각 chunk의 `title`, `noteId`, `chunkId`, `chunkIndex`, `score`, `textLength`, `text`를 함께 기록한다.
