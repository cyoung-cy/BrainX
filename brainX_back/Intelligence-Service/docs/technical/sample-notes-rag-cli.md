...
이 script는 실행 전 `.brainx-local.properties`, OpenAI/Voyage/Qdrant 설정, Qdrant gRPC `localhost:6334` 연결 가능 여부를 preflight로 확인한다. 실패하면 provider 호출 없이 `build/chat-router-captures/<run-id>/summary.json`과 `report.md`에 실패 이유를 기록하고 exit code `2`로 종료한다.
