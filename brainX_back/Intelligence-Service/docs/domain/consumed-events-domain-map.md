# AI-Service Consumed Events Domain Map

이 문서는 `src/main/resources/contracts/knowledge-intelligence.asyncapi.yaml` 기준으로 `AI-Service`가 consumer로 받는 이벤트와 Intelligence Service의 도메인 기능 연결을 정리한다.
...
일부 이벤트는 `AI-Service`가 producer이면서 consumer이기도 하다. 이는 사용자 요청을 받은 synchronous API와 장기 실행 작업 또는 내부 projection 갱신을 분리하기 위한 계약이다.
...
| --- | --- | --- | --- | --- |
| `CaptureReceived` | `brainx.content.ingestion.publishing.capture-received.v1` | `Ingestion-Service` | 지식 탐색, RAG 채팅 | 웹/외부 콘텐츠 캡처가 노트로 연결될 수 있음을 인지하고, `noteId`가 있으면 해당 노트의 검색/RAG 입력 갱신 후보로 본다. |
