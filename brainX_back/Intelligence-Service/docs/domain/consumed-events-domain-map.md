...
| `FolderDeleted` | `brainx.knowledge.workspace.folder-deleted.v1` | 지식 정리 제안, 지식 탐색 | Workspace의 cascade 삭제 payload(`folderIds`, `mode`, `noteIds`)를 기준으로 폴더 projection을 삭제 상태로 바꾸고 포함 노트의 projection, vector index, chunk manifest, summary cache를 제거한다. |
