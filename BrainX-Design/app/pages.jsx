...
function Login({ theme, setTheme, push }) {
  return (
    <AuthShell theme={theme} setTheme={setTheme}>
      ...
      <Field label="비밀번호" type="password" placeholder="••••••••" right={<a href="#" className="text-[12px] text-primary font-normal">비밀번호 찾기</a>} />
      <Btn variant="primary" size="lg" className="w-full mt-2" onClick={()=>navigate('/home')}>로그인</Btn>
...
function Import({ push }) {
  ...
  const run=()=>{ setImporting(true); setProg(0); const iv=setInterval(()=>setProg(p=>{ if(p>=100){clearInterval(iv); setImporting(false); push('가져오기 완료 · 24개 노트','ok'); return 100;} return p+4; }),60); };
  return (
    <div data-route className="max-w-[980px] mx-auto px-6 md:px-8 py-8">
      <h1 className="text-[24px] font-bold tracking-tight mb-1.5">가져오기</h1>
      <p className="text-txt2 text-[14px] mb-7">기존 자료를 옮겨오면 BrainX가 관계를 새로 연결해 드려요.</p>
      <div className="grid md:grid-cols-2 gap-4 mb-6">
        {[{n:'Notion',c:'248 250 252',d:'워크스페이스를 연결해 페이지를 통째로 가져옵니다.'},{n:'Obsidian Vault',c:'139 92 246',d:'마크다운 Vault와 내부 링크를 그대로 유지합니다.'}].map(s=>(
          <Card key={s.n} hover className="p-6">
            <div className="w-12 h-12 rounded-2xl grid place-items-center mb-4" style={{background:`rgb(${s.c} / 0.14)`,color:`rgb(${s.c})`}}><Icon name="import" size={24}/></div>
            <h3 className="text-[17px] font-semibold mb-1.5">{s.n} 가져오기</h3>
            <p className="text-[13.5px] text-txt2 mb-4 leading-relaxed">{s.d}</p>
            <Btn variant="soft" icon="link" onClick={()=>push(`${s.n} 연결 창을 열었어요`)}>{s.n} 연결</Btn>
          </Card>
        ))}
      </div>
      {/* dropzone */}
      <Card className="p-8 border-dashed mb-6">
        <div className="flex flex-col items-center text-center">
          <div className="w-14 h-14 rounded-2xl glass grid place-items-center mb-4 text-primary"><Icon name="upload" size={26}/></div>
          <h3 className="text-[16px] font-semibold mb-1">파일을 끌어다 놓으세요</h3>
          <p className="text-[13px] text-txt3 mb-4">.md · .txt · .pdf · .docx · .zip 지원</p>
          <Btn variant="primary" icon="upload" onClick={run}>파일 선택</Btn>
        </div>
      </Card>
      {/* progress + conflict */}
      {(importing||prog>0) && (
        <Card className="p-5 mb-6">
          <div className="flex items-center justify-between mb-2 text-[13px]"><span className="font-medium text-txt">가져오는 중… notion-export.zip</span><span className="text-txt3 font-mono">{prog}%</span></div>
          <div className="h-2 rounded-full bg-surface2 overflow-hidden"><div className="h-full rounded-full bg-gradient-to-r from-primary to-accent transition-all" style={{width:`${prog}%`}}></div></div>
          {prog>=100 && (
            <div className="mt-4 flex items-center gap-2 text-[13px] text-pink-400"><Icon name="x" size={15}/> 2개 파일 변환 실패 — <button className="text-primary underline">리포트 보기</button></div>
          )}
        </Card>
      )}
      {/* conflict resolver */}
      <Card className="p-5">
        <div className="text-[13px] font-semibold mb-3 flex items-center gap-2"><Icon name="refresh" size={15} className="text-accent"/> 동기화 충돌 해결</div>
        <p className="text-[13px] text-txt2 mb-4">동일한 제목의 노트 3개가 충돌해요. 처리 방식을 선택하세요.</p>
        <div className="flex gap-2">
          <Btn variant="soft" size="sm" onClick={()=>push('최신 수정본을 적용했어요','ok')}>마지막 수정 우선</Btn>
          <Btn variant="soft" size="sm" onClick={()=>push('수동 선택 모드')}>수동으로 선택</Btn>
        </div>
      </Card>
    </div>
  );
...
function ShareNote({ theme, setTheme }) {
  const n=noteById('n1'); const md={ title: n.title, body: SAMPLE_MD.n1 };
  return (
...
Object.assign(window, { AuthShell, Login, Signup, Onboarding, Import, MyPage, Billing, Settings, Support, ShareNote, Admin, SettingsSection });
