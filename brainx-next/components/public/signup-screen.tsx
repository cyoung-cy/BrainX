...
export function SignupScreen() {
  ...
  const allConsented = LEGAL_DOCUMENTS.every(
    (document) => consents[document.consentKey],
  );
  ...
  const emailChecked =
    emailAvailabilityStatus === "available" && checkedEmail === email.trim();
  ...
  const passwordMismatch =
    passwordConfirmTouched && password !== passwordConfirm;
  const requiredOnlyConsentsAccepted =
    consents.termsRequired && consents.privacyRequired;
  const canCheckEmail =
    email.trim().length > 0 &&
  ...
  const handleEmailChange = (value: string) => {
    setEmail(value);
    setEmailFormatError(
      value.trim() && !isValidEmailFormat(value) ? EMAIL_FORMAT_ERROR : "",
    );
    setCheckedEmail("");
  ...
  return (
    <div className="grid h-screen overflow-hidden bg-bg lg:grid-cols-[4fr_6fr]">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(145deg, rgb(var(--surface2)) 0%, rgb(var(--bg2)) 52%, rgb(var(--surface)) 100%)",
        }}
      >
        <SignupGraphCanvas />
        <button
        ...
        <div className="relative z-10 text-[11px] text-txt3">
          © 2026 BrainX 개발팀
        </div>
      </div>
      ...
      <AuthShell>
        <div className="w-full max-w-[600px] px-6 py-5 [&_input]:h-9 [&_input]:rounded-lg [&_input]:text-[14px]">
          <h1 className="mb-1 text-[24px] font-bold tracking-tight text-txt">
            두뇌를 깨우는 1분
          </h1>
          <p className="mb-4 text-[14px] text-txt2">
            무료로 BrainX를 시작하세요.
          </p>
          <div className="mb-1 flex items-start gap-2">
            ...
            <Btn
              variant={emailChecked ? "outline" : "soft"}
              size="sm"
              className="mt-[25px] h-9 shrink-0 px-3 text-[13px]"
          ...
          {emailAvailabilityStatus === "unavailable" ? (
            <p className="mb-2 break-keep text-[12px] leading-snug text-red-400">
          ...
          <div className="mb-2 flex items-start gap-2">
            ...
            <Btn
              variant={codeVerified ? "outline" : "primary"}
              size="sm"
              icon={codeVerified ? "check" : undefined}
              className="mt-[25px] h-9 shrink-0 px-3 text-[13px]"
          ...
          <div className="mb-3 [&>div]:mb-0">
            ...
            <div className="mt-1.5 grid grid-cols-3 gap-1">
              {[1, 2, 3].map((step) => (
                <span
                  key={step}
                  className={`h-1 rounded-full ${passwordStrength.score >= step ? passwordStrength.barClass : "bg-line/50"}`}
                />
              ))}
            </div>
            <p
              className={`mt-1 text-[12px] leading-snug ${passwordStrength.textClass}`}
            >
            ...
            <button
              ...
              className={cx(
                ...
                allConsented
                  ...
                  : "border-line/70 bg-surface/40 hover:border-primary/40",
              )}
            >
              <span
                className={cx(
                    ...
                    <div className="flex items-center gap-2 px-2.5 py-1.5">
                      ...
                      >
                        <span className="min-w-0 flex-1">
                          <span className="block truncate text-[12.5px] font-medium text-txt">
                            {document.shortLabel}
                          </span>
                        ...
                        <span
                          className={cx(
                            ...
                            document.required
                              ...
                              : "border-line/70 bg-surface2/60 text-txt3",
                          )}
                        >
                        ...
                        <Icon
                          ...
                          size={16}
                          className={cx(
                            "shrink-0 text-txt3 transition-transform duration-300",
                            expanded ? "rotate-180" : "",
                          )}
                        />
                      </button>
                        ...
                        <div className="scroll max-h-[96px] space-y-2 overflow-y-auto border-t border-line/50 px-3 py-2 text-[12px] leading-relaxed text-txt2">
                          ...
                          {document.sections.map((section) => (
                            <div key={section.title}>
                              <p className="mb-0.5 text-[12px] font-semibold text-txt">
                                {section.title}
                              </p>
