...
export interface MockContextEntry {
  backlinks: string[];       // 이 노트를 참조하는 노트 제목
  connections: string[];     // 이 노트가 연결하는 노트 제목
  aiSuggestions: string[];   // AI 연결 제안 노트 제목
}
...
export const MOCK_CONTEXT_DATA: Record<string, MockContextEntry> = {
  ...
  neo4j: {
    ...
    connections: ["RAG 파이프라인", "BrainX 아키텍처"],
  ...
  typescript: {
    ...
    connections: ["React 정리", "NextJS 정리"],
    aiSuggestions: ["제네릭 타입 패턴", "타입 체조 실전", "Zod 스키마 연계"],
  },
  react: {
    backlinks: ["TypeScript 정리", "NextJS 정리"],
    connections: ["TypeScript 정리", "NextJS 정리"],
  ...
  "obsidian-link": {
    backlinks: ["BrainX 아키텍처"],
    connections: ["BrainX 아키텍처", "RAG 파이프라인"],
    aiSuggestions: ["Obsidian Graph 분석", "양방향 링크 구현", "Vault 마이그레이션"],
  },
  "rag-flow": {
    backlinks: ["BrainX 아키텍처", "Qdrant 정리"],
    connections: ["Qdrant 정리", "Neo4j 정리"],
    aiSuggestions: ["청킹 전략 비교", "재순위화(Rerank) 기법", "하이브리드 검색 구성"],
  },
  qdrant: {
    backlinks: ["RAG 파이프라인"],
    connections: ["RAG 파이프라인", "Neo4j 정리"],
    aiSuggestions: ["벡터 인덱스 종류", "페이로드 필터링", "Qdrant vs Pinecone 비교"],
  },
  "brainx-arch": {
    backlinks: [],
    connections: ["Neo4j 정리", "RAG 파이프라인", "TipTap 에디터 테스트"],
