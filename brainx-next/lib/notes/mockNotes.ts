...
- Actuator로 모니터링 지원`,
  },
  {
    ...
    category: "backend",
    folderId: "f-study-spring",
    tags: ["backend", "java", "orm"],
    createdAt: D(58),
    updatedAt: D(1),
    content: `# JPA (Java Persistence API)

자바 ORM(Object-Relational Mapping) 표준 명세입니다.
Hibernate가 대표적인 구현체입니다.

## 핵심 개념

### 영속성 컨텍스트 (Persistence Context)
엔티티를 관리하는 메모리상의 공간입니다.
1차 캐시, 동일성 보장, 변경 감지(Dirty Checking)를 제공합니다.

### 엔티티 생명주기
- 비영속(new): 영속성 컨텍스트와 무관한 객체
...
- @OneToMany / @ManyToOne
- @OneToOne / @ManyToMany
- 양방향 연관관계의 주인(연관관계의 주인) 설정 주의

## N+1 문제
연관 엔티티를 조회할 때 추가 쿼리가 N번 발생하는 문제.
Fetch Join, @EntityGraph, batch size 설정으로 해결.

## Spring Data JPA
- Repository 인터페이스만 작성하면 구현체 자동 생성
- 메서드 이름 기반 쿼리(Query Method)
- @Query로 JPQL/네이티브 쿼리 직접 작성`,
  },
  {
    id: "jwt",
    title: "JWT 정리",
...
export const MOCK_CONTEXT_DATA: Record<string, MockContextEntry> = {
  ...
  jpa: {
    backlinks: ["Spring 정리"],
    connections: ["Spring 정리", "PostgreSQL 정리"],
    aiSuggestions: ["N+1 문제 해결 전략", "QueryDSL 연동", "영속성 컨텍스트 심화"],
  },
  jwt: {
    backlinks: ["Spring 정리"],
