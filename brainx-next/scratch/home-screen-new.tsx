  ...
  return (
    <>
      <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        {kpis.map((kpi, idx) => (
          <div key={kpi.label} className="flex flex-col justify-between rounded-2xl border border-line/60 bg-surface/80 p-4 transition-colors hover:bg-surface">
            ...
            <div>
              ...
              <div className="mt-1 text-[12px] font-medium text-txt3">{kpi.label}</div>
            </div>
            <div className="mt-3 h-[3px] w-full overflow-hidden rounded-full bg-line/50">
        ...
        <div className="flex flex-col rounded-2xl border border-line/60 bg-surface/40 overflow-hidden">
          <div className="flex items-center justify-between border-b border-line/60 bg-surface p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[14px] font-semibold text-txt">나의 지식 인사이트</span>
            </div>
          ...
          <div className="p-5">
            <div
              className="mb-3 rounded-[0.4rem] border border-line/50 bg-surface2/30 p-4 text-[13px] leading-relaxed text-txt2"
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-[0.4rem] border border-line/60 bg-surface2/40 p-3">
                <div className="text-[18px] font-semibold text-txt">{notes.length}개</div>
                <div className="mt-0.5 text-[11px] text-txt3">분석된 노트</div>
                <div className="mt-1.5 inline-block rounded-[0.4rem] bg-emerald-500/15 px-1.5 py-0.5 text-[10px] font-medium text-emerald-500">+3 이번 주</div>
              </div>
              <div className="rounded-[0.4rem] border border-line/60 bg-surface2/40 p-3">
                <div className="text-[18px] font-semibold text-txt">{totalLinks}개</div>
                <div className="mt-0.5 text-[11px] text-txt3">지식 연결</div>
                <div className="mt-1.5 inline-block rounded-[0.4rem] bg-emerald-500/15 px-1.5 py-0.5 text-[10px] font-medium text-emerald-500">+5 이번 주</div>
              </div>
              <div className="rounded-[0.4rem] border border-line/60 bg-surface2/40 p-3">
                <div className="text-[18px] font-semibold text-txt">{topClusters.length}개</div>
                <div className="mt-0.5 text-[11px] text-txt3">핵심 주제군</div>
              </div>
              <div className="rounded-[0.4rem] border border-line/60 bg-surface2/40 p-3">
                <div className="text-[18px] font-semibold text-txt">{Math.round(totalWords / Math.max(notes.length, 1))}자</div>
                <div className="mt-0.5 text-[11px] text-txt3">노트 평균 분량</div>
              </div>
            </div>
          </div>

          <div className="flex items-center justify-between border-b border-t border-line/60 bg-surface p-4">
            <div className="flex items-center gap-2">
              <div className="grid h-[26px] w-[26px] place-items-center rounded-[0.4rem] bg-emerald-500/15 text-emerald-500">
                <Icon name="link" size={14} />
              </div>
              <span className="text-[14px] font-semibold text-txt">주제 지도</span>
            </div>
          ...
          <div className="p-4 flex-1 flex flex-col">
            <div className="relative h-[220px] w-full overflow-hidden rounded-[0.4rem] border border-line/60 bg-surface/50">
              {topicView === "bubble" ? (
                <svg viewBox="0 0 320 210" className="h-full w-full">
                  <defs>
                    {bubbles.map((b, i) => (
                      <radialGradient key={`glow-${i}`} id={`glow-${i}`} cx="50%" cy="50%" r="50%">
                        <stop offset="0%" stopColor={`rgb(${b.color})`} stopOpacity="0.25" />
                        <stop offset="100%" stopColor={`rgb(${b.color})`} stopOpacity="0" />
                      </radialGradient>
                    ))}
                  </defs>
                  {bubbles.map((b, i) => (
                    <circle key={`halo-${i}`} cx={b.left * 3.2} cy={b.top * 2.1} r={b.size * 0.8} fill={`url(#glow-${i})`} />
                  ))}
                  {bubbles.map((b, i) => {
        ...
        <div className="flex flex-col rounded-2xl border border-line/60 bg-surface/40 overflow-hidden">
          <div className="flex items-center justify-between border-b border-line/60 bg-surface p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[14px] font-semibold text-txt">인사이트 요약</span>
            </div>
            <button className="flex items-center gap-1 text-[11px] text-txt3 hover:text-txt">
          ...
          <div className="flex-1 bg-surface2/20">
            {insights.map((insight, index) => (
              <div key={index} className="flex items-stretch gap-3 border-b border-line/40 bg-surface/60 p-4 transition-colors hover:bg-surface cursor-default">
                <div className="w-[3px] shrink-0 rounded-full" style={{ background: insight.color }} />
                <div className="flex-1">
                  <div className="mb-1 text-[10px] font-semibold uppercase tracking-wider" style={{ color: insight.color }}>{insight.tag}</div>
          ...
          <div className="border-t border-line/60 bg-surface p-4">
            <div className="mb-2.5 text-[11px] font-semibold uppercase tracking-wider text-txt3">다시 보면 좋은 노트</div>
            <div className="flex items-start gap-3 rounded-[0.4rem] border border-accent/20 bg-accent/5 p-3.5">
    ...
    <div data-route className="mx-auto max-w-[1100px] px-6 py-6 md:px-8 lg:py-8">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4 border-b border-line/60 pb-5">
        <div>
          ...
          <h1 className="text-[26px] font-semibold tracking-tight text-txt">
            좋은 아침이에요,<br />
